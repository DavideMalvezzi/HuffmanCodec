...
GTK_HANDLER(void onCompressClick){
	...
	if(fileList != NULL){
		...
		if(res == GTK_RESPONSE_ACCEPT){
			//GtkWidget* compressingDialog = GTK_WIDGET(gtk_builder_get_object(builder, "compressingDialog"));
			//gtk_widget_show_all(compressingDialog);
			...
			//gtk_widget_hide(compressingDialog);
		}
